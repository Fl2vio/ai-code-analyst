...
function getLearningStats() {
  ...
  const patternStorePath = path.join(
    CWD,
    ".claude-flow",
    "data",
    "patterns.json",
  ...
  if (patterns === 0) {
    const autoStorePath = path.join(
      CWD,
      ".claude-flow",
      "data",
      "auto-memory-store.json",
    );
    try {
...
function getV3Progress() {
  ...
  const dddData = readJSON(
    path.join(CWD, ".claude-flow", "metrics", "ddd-progress.json"),
  );
...
function getSecurityStatus() {
  const auditData = readJSON(
    path.join(CWD, ".claude-flow", "security", "audit-status.json"),
  );
  if (auditData) {
...
function getSwarmStatus() {
  ...
  const swarmStatePath = path.join(
    CWD,
    ".claude-flow",
    "swarm",
    "swarm-state.json",
  ...
  if (swarmState) {
    ...
    if (age < staleThresholdMs) {
      return {
        activeAgents:
          (swarmState.agents && swarmState.agents.length) ||
  ...
  const activityData = readJSON(
    path.join(CWD, ".claude-flow", "metrics", "swarm-activity.json"),
  );
  if (activityData && activityData.swarm) {
    const updatedAt =
      activityData.timestamp ||
      (activityData.swarm && activityData.swarm.timestamp);
    const age = updatedAt ? now - new Date(updatedAt).getTime() : Infinity;
    if (age < staleThresholdMs) {
      return {
        ...
        coordinationActive:
          activityData.swarm.coordination_active ||
          activityData.swarm.active ||
          false,
      };
    }
  }
...
function getSystemMetrics() {
  ...
  const learningData = readJSON(
    path.join(CWD, ".claude-flow", "metrics", "learning.json"),
  );
  ...
  if (
    learningData &&
    learningData.intelligence &&
    learningData.intelligence.score !== undefined
  ) {
    intelligencePct = Math.min(
      100,
      Math.floor(learningData.intelligence.score),
    );
  } else {
    // Use real data only — patterns from actual store, vectors from actual DB
    const fromPatterns =
      learning.patterns > 0
        ? Math.min(100, Math.floor(learning.patterns / 20))
        : 0;
    const fromVectors =
      agentdb.vectorCount > 0
        ? Math.min(100, Math.floor(agentdb.vectorCount / 20))
        : 0;
  ...
  if (
    activityData &&
    activityData.processes &&
    activityData.processes.estimated_agents
  ) {
...
function getAgentDBStats() {
  ...
  try {
    ...
    if (ranked && ranked.entries && ranked.entries.length > vectorCount)
      vectorCount = ranked.entries.length;
  } catch {
    /* ignore */
  }
...
function generateStatusline() {
  ...
  header +=
    (swarm.coordinationActive ? c.brightCyan : c.dim) +
    "\u25CF " +
    c.brightCyan +
    git.name +
    c.reset;
  ...
  if (agentdb.hasHnsw && agentdb.vectorCount > 0) {
    const speedup =
      agentdb.vectorCount > 10000
        ? "12500x"
        : agentdb.vectorCount > 1000
          ? "150x"
          : "10x";
    ...
    perfIndicator =
      c.brightYellow + "\uD83D\uDCDA " + pk + " patterns" + c.reset;
  } else {
  ...
  lines.push(
    c.brightYellow +
      ...
      " [" +
      agentsColor +
      String(swarm.activeAgents).padStart(2) +
      c.reset +
      "/" +
      c.brightWhite +
      swarm.maxAgents +
      ...
      intellColor +
      "\uD83E\uDDE0 " +
      String(system.intelligencePct).padStart(3) +
      "%" +
      c.reset,
  );
  ...
  lines.push(
    c.brightPurple +
      ...
      dddColor +
      "\u25CF" +
      String(progress.dddProgress).padStart(3) +
      "%" +
      c.reset +
      "  " +
      c.dim +
  ...
  if (integration.mcpServers.total > 0) {
    ...
    integStr +=
      ...
      mcpCol +
      "\u25CF" +
      integration.mcpServers.enabled +
      "/" +
      integration.mcpServers.total +
      c.reset;
  }
  if (integration.hasDatabase)
    integStr +=
      (integStr ? "  " : "") + c.brightGreen + "\u25C6" + c.reset + "DB";
  if (integration.hasApi)
    integStr +=
      (integStr ? "  " : "") + c.brightGreen + "\u25C6" + c.reset + "API";
