...
async function render({ model, el }) {
  ...
  const reembed = async () => {
    ...
    for (const selectionName of Object.keys(model.get("_vl_selections"))) {
      ...
      api.view.addSignalListener(
        selectionName,
        lodashDebounce(selectionHandler, wait, debounceOpts),
      );
    ...
    for (const paramName of Object.keys(model.get("_params"))) {
      const paramHandler = (_, value) => {
        const newParams =
          JSON.parse(JSON.stringify(model.get("_params"))) || {};
      ...
      api.view.addSignalListener(
        paramName,
        lodashDebounce(paramHandler, wait, debounceOpts),
      );
    ...
    for (const watch of model.get("_js_watch_plan") ?? []) {
      if (watch.namespace === "data") {
        ...
        addDataListener(
          ...
          watch.name,
          watch.scope,
          lodashDebounce(dataHandler, wait, debounceOpts),
        );
        ...
        addSignalListener(
          ...
          watch.name,
          watch.scope,
          lodashDebounce(signalHandler, wait, debounceOpts),
        );
      }
    }
...
export function addSignalListener(view, name, scope, handler) {
  let signal_op = lookupSignalOp(view, name, scope);
  return addOperatorListener(view, name, signal_op, handler);
}

export function addDataListener(view, name, scope, handler) {
  let dataset = dataRef(view, name, scope).values;
  return addOperatorListener(view, name, dataset, handler);
}
