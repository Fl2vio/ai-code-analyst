...
export default function Navbar() {
  ...
  return (
    <>
      ...
      >
        {/* Logo + badge */}
        <a href="#demo" style={{ textDecoration: 'none', display: 'flex', alignItems: 'center', gap: '10px' }}>
          <svg width="32" height="32" viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            {/* Chest patch */}
            <ellipse cx="32" cy="50" rx="7" ry="4" fill="#ffd4b3" opacity="0.6"/>
            {/* Glow ring */}
            <ellipse cx="32" cy="36" rx="22" ry="18" fill="none" stroke="#00e5ff" strokeWidth="1.2" opacity="0.35"/>
          </svg>
          <span style={{
          ...
          {bugInfo && (
            ...
            >
              ...
              <span style={{
                ...
                fontSize: '0.62rem',
                color: bugInfo.error ? '#ef4444' : bugInfo.count > 0 ? '#f59e0b' : '#10b981',
        ...
        <div style={{ display: 'flex', gap: '28px', alignItems: 'center' }} className="desktop-nav">
          {links.map(l => (
            <div key={l.href} style={{ position: 'relative', paddingBottom: '4px' }}>
              <a
                href={l.href}
                style={{
                  fontFamily: 'IBM Plex Mono, monospace',
                ...
                onMouseEnter={e => { if (!isActive(l.href)) e.currentTarget.style.color = '#e2e8f0' }}
                onMouseLeave={e => { if (!isActive(l.href)) e.currentTarget.style.color = '#94a3b8' }}
              >
                {l.label}
              </a>
              {isActive(l.href) && (
          ...
          <motion.button
            ...
            style={{
              ...
              border: dark ? '1px solid rgba(0,229,255,0.25)' : '1px solid rgba(0,0,0,0.12)',
              borderRadius: '8px',
              width: '34px',
              height: '34px',
              display: 'flex',
        ...
        >
          <div style={{ width: 22, height: 2, background: menuOpen ? '#00e5ff' : '#94a3b8', marginBottom: 5, transition: 'all 0.2s', transform: menuOpen ? 'rotate(45deg) translate(5px, 5px)' : 'none' }} />
          <div style={{ width: 22, height: 2, background: menuOpen ? 'transparent' : '#94a3b8', marginBottom: 5, transition: 'all 0.2s' }} />
          <div style={{ width: 22, height: 2, background: menuOpen ? '#00e5ff' : '#94a3b8', transition: 'all 0.2s', transform: menuOpen ? 'rotate(-45deg) translate(5px, -5px)' : 'none' }} />
        </button>
      </motion.nav>
        ...
        {menuOpen && (
          ...
          >
            {links.map(l => (
              ...
              >
                {isActive(l.href) && <span style={{ width: 4, height: 4, borderRadius: '50%', background: '#00e5ff', display: 'inline-block' }} />}
                {l.label}
              </a>
            ))}
            <a
